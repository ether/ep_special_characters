# Contributing to ep_special_characters

Thanks for helping improve `ep_special_characters` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_special_characters` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_special_characters from your local clone
pnpm run plugins i --path ../ep_special_characters

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_special_characters/` are picked up after a server restart.
...
* Etherpad version, ep_special_characters version, browser + OS
* Steps to reproduce
